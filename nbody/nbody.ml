...
let () =
  ...
  for i = 0 to n - 1 do
    ...
    if i mod n_mod = 0
    then (
      xy.(!i') <- bodies.(2).x, bodies.(2).y;
      incr i')
    else ()
  done;
  let series = Gnuplot.Series.lines_xy (Array.to_list xy) in
  let gp = Gnuplot.create () in
  Gnuplot.plot gp series ~output:(Gnuplot.Output.create (`Png_cairo "nbody.png"));
  Gnuplot.close gp;
  Printf.printf "%.9f\n" (energy bodies)
;;
